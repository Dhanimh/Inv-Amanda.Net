...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif (isKodeBarangExists($conn, $kode_barang, $id)) {
    ...
    if (empty($errors)) {
        ...
        if (updateBarang($conn, $id, $data)) {
            logActivity($user['id'], 'Mengubah data barang', "Barang: $nama_barang ($kode_barang)");
            setAlert('success', 'Barang berhasil diperbarui!');
            header("Location: index");
            exit();
        } else {
            $errors[] = "Gagal memperbarui barang: " . mysqli_error($conn);
        }
    }
    
    // Use POST data for form
...
<div class="lg:ml-64 pt-16">
    <div class="p-6">
        ...
        <div class="mb-6">
            ...
            <p class="text-gray-600 mt-1">Perbarui informasi barang</p>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="kategori" required
                            ...
                            <option value="Router" <?= $barang['kategori'] == 'Router' ? 'selected' : '' ?>>Router</option>
                            <option value="Access Point" <?= $barang['kategori'] == 'Access Point' ? 'selected' : '' ?>>Access Point</option>
                            <option value="Switch" <?= $barang['kategori'] == 'Switch' ? 'selected' : '' ?>>Switch</option>
                            <option value="Kabel" <?= $barang['kategori'] == 'Kabel' ? 'selected' : '' ?>>Kabel</option>
                            <option value="ONT" <?= $barang['kategori'] == 'ONT' ? 'selected' : '' ?>>ONT</option>
                            <option value="Modem" <?= $barang['kategori'] == 'Modem' ? 'selected' : '' ?>>Modem</option>
                            <option value="Antena" <?= $barang['kategori'] == 'Antena' ? 'selected' : '' ?>>Antena</option>
                    ...
                    <div>
                        ...
                        <input type="number" name="stok" required min="0"
                               value="<?= $barang['stok'] ?>"
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                        <p class="text-xs text-gray-500 mt-1">Update stok manual di sini atau via transaksi</p>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="deskripsi" rows="4"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"><?= $barang['deskripsi'] ?></textarea>
                    </div>
                </div>
